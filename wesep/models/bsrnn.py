...
# modified from wesep.utils.funcs: compute_fbank
...
def _apply_cmvn(mat, norm_mean=True, norm_var=False):
    if norm_mean:
...
class BSRNN(nn.Module):
    ...
    def __init__(
        ...
        self.oracle_enroll_prob = oracle_enroll_prob
        ...
        if joint_training:
            if spk_model is None:
                ...
                # Lazy import: DAE-BSRNN does not need wespeaker.
    ...
    def forward_spk_encoder(self, spk_emb_input):
        predict_speaker_label = torch.tensor(0.0).to(
            spk_emb_input.device)  # dummy
        
        # when self.spk_encoder is None, spk_embedding should also be None 
        if self.spk_encoder is None:
            return predict_speaker_label, None
        ...
        return predict_speaker_label, spk_embedding
    ...
    def forward(self, wav_input, spk_emb_input, additional = {}):
        '''
            wav_input shape: (B, C, T)
            spk_emb_input shape: (B, C, T')
        '''
        # breakpoint()

        # apply oracle enrollment with 50% probability
        if "targets" in  additional and random.random() < self.oracle_enroll_prob:
            oracle_enroll = []
            targets = additional["targets"]
            for spk_idx in range(wav_input.shape[0]):
                _fank_feat = _compute_fbank(waveform=targets[spk_idx].unsqueeze(0))
                _fank_feat = _apply_cmvn(_fank_feat)
                oracle_enroll.append(_fank_feat)
            oracle_enroll = torch.stack(oracle_enroll, 0).to(wav_input.device)
            spk_emb_input = oracle_enroll
        ...
        # forward spk_embedding
        predict_speaker_label, spk_embedding = self.forward_spk_encoder(spk_emb_input)

        # forward separator
        sep_output = self.separator(subband_feature, spk_embedding, torch.tensor(self.nch))
        ...
        return s, predict_speaker_label
