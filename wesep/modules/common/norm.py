...
class MHCAFuseLayer(nn.Module):
    def __init__(self,
        ...
        self.q_mlp = nn.Sequential(
            nn.Linear(input_qdim, embed_dim),
            nn.LayerNorm(embed_dim),
            nn.GELU(),
            
        )
        self.kv_mlp = nn.Sequential(
            nn.Linear(input_kvdim, embed_dim),
            nn.LayerNorm(embed_dim),
            nn.GELU(),
        )
    ...
    def forward(self, q: torch.Tensor, kv: torch.Tensor, key_padding_mask: torch.Tensor = None):
        projected_q = self.q_mlp(q)
        projected_kv = self.kv_mlp(kv)
        attn_output, attn_output_weights = self.mha(projected_q, projected_kv, projected_kv, key_padding_mask)
        return attn_output